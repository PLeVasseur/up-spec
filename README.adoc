...
+
Native frame APIs, serializer-neutral payload metadata, and owned/zero-copy capability splits are defined in xref:up-l1/wire-format-and-frame-api.adoc[Wire Format and Frame API] and xref:up-l1/owned-and-zero-copy-transport-api.adoc[Owned and Zero-Copy Transport API]. These documents describe the transition away from generated transport envelopes for language bindings that expose native frame APIs.
