...
+
Native frame APIs, payload encoding metadata, and owned/zero-copy transport capability terminology are defined in xref:up-l1/wire-format-and-frame-api.adoc[Wire Format and Frame API] and xref:up-l1/owned-and-zero-copy-transport-api.adoc[Owned and Zero-Copy Transport API]. These documents describe compatibility rules for language bindings that expose native frame APIs in addition to the ordinary message transport API.
...
Along with the specification itself, this repository contains Gherkin Features and Scenarios, defined in files with a `.feature` suffix, which can be used with test frameworks like https://cucumber.io/[Cucumber] to assert compliance of implementations with the uProtocol specification. Not all parts of the specification are covered (yet), but we intend to add Scenario descriptions so that more and more of the requirements defined in the specification will be covered over time.
