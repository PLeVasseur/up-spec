...
NOTE: `UMessage` remains the protocol message data model and the compatibility serialization shape. Transport language bindings MAY also expose native frame APIs that carry binding-native uProtocol attributes, payload encoding metadata, and payload bytes without requiring `UMessage` to be the transport envelope. See xref:../up-l1/wire-format-and-frame-api.adoc[Wire Format and Frame API].
