...
NOTE: `UMessage` remains the protocol message data model and compatibility serialization shape. Native transport APIs MAY expose language-binding frame types that carry native `UAttributes`, `PayloadEncoding`, and payload bytes without requiring `UMessage` as the transport envelope. See xref:../up-l1/wire-format-and-frame-api.adoc[Wire Format and Frame API].
