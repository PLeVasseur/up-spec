...
|no
|none
a|The encoding of the message payload, represented by an identifier whose
meaning is defined in the
xref:payload_encoding_registry.adoc[payload-encoding registry].
...
If present, the property *MUST* contain a value in the range `0x0000` to
`0xFFFF`. Every value in that range is structurally valid: a component *MUST
NOT* reject a message because the value is reserved, unassigned, private, or
not recognized by that component.

A component that needs to interpret the payload *MUST* reject it if the
encoding cannot be determined or is not supported by that component. A value of
`0x0000` indicates that an applicable binding, service, or schema contract
defines the concrete encoding; a component that interprets such a payload
*MUST* have that contract and *MUST NOT* infer the encoding from the payload
data.
--

Which identifiers a producer may originate is an allocation policy of the
registry, not a validation rule of this attribute; see
xref:payload_encoding_registry.adoc#allocation[Allocation and Origination].
