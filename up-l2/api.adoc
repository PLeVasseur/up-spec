...
[#communication-roles]
The Communication Layer owns publish, subscribe, notification, and RPC roles. It consumes uP-L1 transport endpoints and does not own selected wire identity, metadata codec choice, payload codec choice, or whole-frame envelope layout. A language library can offer direct uP-L1 capability APIs alongside the Communication Layer APIs when applications or transport implementations need transport-layer representation control.
