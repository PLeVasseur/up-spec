...
Calls to the uSubscription service `Subscribe()` operation *MUST* return a failure status message with {ucode_proto_ref} `InvalidArgument` in case a `topic` UURI violates the xref:usubscription-topics[topic UURI rules].
--
...
Calls to the uSubscription service `Unsubscribe()` operation *MUST* return a failure status message with {ucode_proto_ref} `InvalidArgument` in case a `topic` UURI violates the xref:usubscription-topics[topic UURI rules].
--
|===
...
The uSubscription service `FetchSubscriptions()` operation *MUST* return a failure status message with {ucode_proto_ref} `InvalidArgument` in case a topic filter UURI is not a valid {uuri_ref}.
--
...
The uSubscription service `FetchSubscriptions()` operation *MUST* return a failure status message with {ucode_proto_ref} `InvalidArgument` in case a subscriber filter UURI is not a valid {uuri_ref}.
--
|===
...
When receiving a `Reset()` call from a source that is not another uSubscription services (i.e. from source URIs where uEntity type does not equal `0x0`), uSubscription service *MUST* return a failure status message with {ucode_link} `PermissionDenied`.
