...
MQTT 5 native-frame bindings that carry stable-container payloads *MUST* preserve the `up.stable-container` custom payload encoding metadata and application payload bytes exactly. MQTT broker delivery is an owned/buffered path and does not provide loan-backed stable-container typed borrowing.
