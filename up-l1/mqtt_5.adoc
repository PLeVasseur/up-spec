...
A uProtocol message consists of _UAttributes_ and optional payload. Native frame language bindings also carry explicit _PayloadEncoding_ metadata for the payload wire representation. The following sections define how these are mapped to/from an MQTT 5 PUBLISH packet.
...
* *MUST* be set to the xref:../basics/uri.adoc#uri-definition[string serialization of the UUri]
...
a| The message's priority class as defined in xref:../basics/upriority.adoc[Message Priority]
...
a| The service consumer's permission level as defined in xref:../up-l2/permissions.adoc#_code_based_access_permissions_caps[Code-Based uEntity Access Permissions (CAPs)]

* *MUST* be set to the xref:../up-l2/permissions.adoc#_code_based_access_permissions_caps[CAP] value's decimal string representation. 
...
* Generated-envelope language bindings *MUST* set this to the link:../up-core-api/uprotocol/v1/uattributes.proto[UPayloadFormat enum] integer value's decimal string representation.
* Native-frame language bindings *MUST* set this to the selected payload encoding's content type when one is known and *MUST NOT* set it for frames without payload.

Note that the generated enum's integer value is used instead of the _uprotocol.mime_type_ option's value in generated-envelope bindings in order to reduce the overall size of the PUBLISH packet.

| Standard `PayloadEncoding`
| User Property [key: _12_]
a| Integer value of the upstream `UPayloadFormat`.

* Native-frame language bindings *MUST* set this property to the standard `UPayloadFormat` value when standard payload encoding is present and *MUST NOT* set it for frames without payload or with custom payload encoding.
* Receivers that support native frames *MUST* preserve this value exactly.

| Custom `PayloadEncoding.id`
| User Property [key: _uencodingid_]
a| Stable identifier for a native-only custom payload codec family or byte-compatible layout.

* Native-frame language bindings *MUST* set this property when custom payload encoding is present and *MUST NOT* set it for standard payload encoding or frames without payload.
* Custom payload encoding *MUST* also set the MQTT Content Type property to `PayloadEncoding.content_type`.
* Receivers that support native frames *MUST* preserve custom encoding IDs and content types exactly.
...
For native-frame language bindings, the MQTT 5 PUBLISH payload *MUST* contain exactly the application payload bytes produced by the selected serializer. Native frame metadata is carried in MQTT properties and *MUST NOT* be prepended to or otherwise exposed as part of the application payload.

Frames without payload *MUST* be sent with an empty PUBLISH payload and without native-frame payload encoding properties. Receivers *MUST* reject a non-empty PUBLISH payload that lacks native-frame payload encoding properties, unless the language binding explicitly documents a raw-bytes default.
--

MQTT 5 is an owned native-frame transport in this revision. MQTT 5 native-frame bindings that carry stable-container payloads *MUST* preserve the `up.stable-container` custom payload encoding metadata and application payload bytes exactly. MQTT broker delivery is an owned/buffered path and does not provide loan-backed stable-container typed borrowing. MQTT 5 transport implementations *MUST NOT* advertise direct true-zero-copy transmit, direct true-zero-copy receive, or loan-backed stable-container typed borrowing.

=== Native-Frame Conformance Expectations

MQTT 5 native-frame conformance tests *MUST* cover invalid native attributes, standard payload encoding preservation, custom payload encoding preservation, stable-container metadata preservation without typed borrowing, missing encoding rejection for non-empty PUBLISH payloads, absent-payload normalization for empty PUBLISH payloads without encoding, and exact application-payload delivery without exposing MQTT properties as payload bytes. Broker-dependent integration smoke tests *SHOULD* run where an MQTT 5 broker fixture is available.

[.specitem,oft-sid="dsn~up-transport-mqtt5-native-frame-conformance~1",oft-needs="impl,utest",oft-tags="TransportLayerImpl"]
--
MQTT 5 native-frame bindings *MUST* prove owned transport semantics: metadata and payload bytes are preserved through MQTT properties and PUBLISH payloads, while zero-copy loans, zero-copy receive leases, and loan-backed stable-container typed borrows are not advertised.
--
...
2. the upper-case https://www.rfc-editor.org/rfc/rfc4648#section-8[base16 encoding] of the uEntity type identifier with all leading `0` characters omitted.
...
2. the upper-case https://www.rfc-editor.org/rfc/rfc4648#section-8[base16 encoding] of the uEntity instance identifier with all leading `0` characters omitted.
...
2. the upper-case https://www.rfc-editor.org/rfc/rfc4648#section-8[base16 encoding] of the uEntity major version with all leading `0` characters omitted.
...
2. the upper-case https://www.rfc-editor.org/rfc/rfc4648#section-8[base16 encoding] of the resource identifier with all leading `0` characters omitted.
...
* re-subscribe to all previously subscribed topics, if the broker indicates that https://docs.oasis-open.org/mqtt/mqtt/v5.0/os/mqtt-v5.0-os.html#_Toc3901078[no session is present] after successful reconnection.
