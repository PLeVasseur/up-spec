...
MQTT 5 is an owned native-frame transport in this revision. Preserving uProtocol metadata and payload bytes in MQTT properties and PUBLISH payloads does not provide a transport loan or receive lease. MQTT 5 transport implementations *MUST NOT* advertise direct true-zero-copy transmit, direct true-zero-copy receive, or loan-backed stable-container typed borrowing.
