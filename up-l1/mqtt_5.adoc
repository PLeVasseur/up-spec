...
[#mqtt5-transport-capability-profile]
== Transport Capability Profile

The MQTT 5 binding is a brokered uP-L1 compatibility message transport profile. It maps uProtocol metadata to MQTT 5 PUBLISH properties, payload bytes to the PUBLISH payload, and routing/filtering information to MQTT topics.

The MQTT 5 profile:

* *MUST* support the xref:README.adoc#compatibility-message-transport[compatibility message transport] semantics defined by uP-L1.
* *MUST* preserve uProtocol metadata using the PUBLISH property mapping in <<UAttributes>> and payload bytes using <<Payload>>.
* *MUST* treat MQTT topic names as transport-local routing and filtering representation, not as Communication Layer ownership of wire identity.
* *MUST* document broker acknowledgement, message expiry, topic subscription, and retained-message behavior that can affect delivery semantics.
* *MUST NOT* claim native-frame or loan-backed zero-copy capability unless a concrete binding documents a separate capability surface and its copy boundaries.
