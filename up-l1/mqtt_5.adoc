...
A uProtocol message consists of _UAttributes_ and optional payload. Native frame language bindings also carry explicit _UEncoding_ metadata for the payload wire representation. The following sections define how these are mapped to/from an MQTT 5 PUBLISH packet.
...
* Native-frame language bindings *MUST* set this to `UEncoding.content_type` when payload encoding is present and *MUST NOT* set it for frames without payload.
...
| `UEncoding.format_id`
| User Property [key: _uformatid_]
a| Stable identifier for the payload codec family or IDL binding.

* Native-frame language bindings *MUST* set this property to `UEncoding.format_id` when payload encoding is present and *MUST NOT* set it for frames without payload.
* Receivers that support native frames *MUST* preserve this value exactly.

| `UEncoding.schema_ref`
| User Property [key: _uschemaref_]
a| Optional stable schema identity for the payload.

* Native-frame language bindings *MUST* set this property if, and only if, payload encoding is present and `UEncoding.schema_ref` is present.
* Empty schema references *MUST* be treated as absent.
* Receivers that support native frames *MUST* preserve non-empty values exactly.
