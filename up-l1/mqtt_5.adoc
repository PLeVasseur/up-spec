...
MQTT 5 native-frame conformance tests SHOULD cover standard payload encoding preservation, custom payload encoding preservation, stable-container metadata preservation without typed borrowing, missing encoding rejection for non-empty PUBLISH payloads, absent-payload normalization for empty PUBLISH payloads without encoding, and exact application-payload delivery without exposing MQTT properties as payload bytes.
