...
* Native-frame language bindings *MUST* set this to `UEncoding.content_type`.
...
* Native-frame language bindings *MUST* set this property to `UEncoding.format_id`.
...
* Native-frame language bindings *MUST* set this property if, and only if, `UEncoding.schema_ref` is present.
...
For native-frame language bindings, the MQTT 5 PUBLISH payload *MUST* contain exactly the application payload bytes produced by the selected `WireFormat` serializer. Native frame metadata is carried in MQTT properties and *MUST NOT* be prepended to or otherwise exposed as part of the application payload.
--
