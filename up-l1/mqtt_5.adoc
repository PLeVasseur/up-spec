...
MQTT 5 binding supports the *classic* transport family only. Frame-oriented
family operations *MUST* fail with an explicit, typed unsupported-operation
status. Metadata carriage pattern — Mapped fields: attributes ride MQTT 5
user properties keyed by protobuf field number, including the open
payload-encoding identity fields (13, 14, 15).

=== UFrame Binding Requirements

An implementation that carries xref:../basics/uframe.adoc[UFrames] over MQTT 5
*MUST* define a binding profile. The binding *MUST* declare whether UFrame
metadata is carried in MQTT user properties, inside a `UPFE` whole-frame
envelope payload, or by another explicitly named placement.
...
and routing mirrors that do not match the MQTT topic selected by the binding.
