...
The serialization-neutral frame model is defined in xref:wire-format-and-frame-api.adoc[Wire Format and Frame API]. Owned-buffer and zero-copy transport capabilities are defined in xref:owned-and-zero-copy-transport-api.adoc[Owned and Zero-Copy Transport API].
