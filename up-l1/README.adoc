...
The serialization-neutral native frame model, payload encoding metadata, serializer compatibility rules, and stable-container compatibility requirements are defined in xref:wire-format-and-frame-api.adoc[Wire Format and Frame API]. Owned-buffer and zero-copy transport capability terms are defined in xref:owned-and-zero-copy-transport-api.adoc[Owned and Zero-Copy Transport API].
