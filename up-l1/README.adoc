...
The `UTransport` API specified in this document exchanges fully serialized
`UMessage`s and is the *classic transport family*. uProtocol additionally
defines two frame-oriented access models — the *owned-frame* and *zero-copy*
families — together with the *transport core* layering that lets one native
transport implementation serve all families and all
xref:../basics/uframe.adoc[UFrame] wire formats. Those families, the
selected-wire operating rules, and the constraints that keep diverse
transport technologies interoperable are specified in
xref:transport_families.adoc[Transport Families and Transport Cores]. Nothing
in that document changes the conformance requirements for the classic API
specified here.
...
* xref:iceoryx2.adoc[*Eclipse iceoryx2*]
* xref:lola.adoc[*Eclipse S-CORE LoLa*]
