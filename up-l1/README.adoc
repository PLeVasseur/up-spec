...
The serialization-neutral native frame model, wire-format terminology, metadata codec rules, payload codec family rules, and stable-container compatibility requirements are defined in xref:wire-format-and-frame-api.adoc[Wire Format and Frame API]. Owned-buffer, wire-bound, and zero-copy transport capability terms are defined in xref:owned-and-zero-copy-transport-api.adoc[Owned and Zero-Copy Transport API].
