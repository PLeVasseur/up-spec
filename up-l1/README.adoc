...
[#native-frame-zero-copy-scope]
== Native Frame and Zero-Copy Capability Scope

Language libraries *MAY* expose native-frame transport APIs in addition to the abstract `UTransport` API shown below. A native frame carries uProtocol attributes, payload encoding metadata and payload bytes without requiring a generated `UMessage` envelope at every transport boundary. Native-frame APIs are conformant only if they preserve the same uProtocol message semantics defined by this specification.

This section defines the terminology used by native-frame and zero-copy bindings. It does not require every transport protocol to provide every capability.
...
| A transport API whose send and receive values own their payload bytes. The implementation preserves uProtocol metadata and payload bytes, but it does not expose loan-backed typed payload borrows.
...
| A routing path that avoids unnecessary intermediate payload buffers but still copies payload bytes at least once, for example into an egress transmit loan.
...
| A path where the application borrows payload bytes directly from a receive lease backed by the transport or shared-memory runtime, without allocation, payload coalescing or copied fallback storage.
...
--
Native-frame transport libraries *MUST* document which of the above transport classes they implement and *MUST NOT* describe owned native-frame transports, copying adapters, or copy-minimized routes as direct true-zero-copy transmit or receive paths.
--

[.specitem,oft-sid="dsn~utransport-stable-container-borrow-scope~1",oft-needs="impl,utest",oft-tags="TransportLayerImpl"]
--
Language bindings that expose fixed-size stable-container typed payload borrowing *MUST* expose the safe typed borrow only through a loan-backed receive API. In the Rust binding this API is a stable-specific loan-backed receive method such as `borrow_stable_payload<T>()` on the loan-backed receive trait. Owned native-frame transports *MAY* preserve stable-container payload bytes and stable-container metadata, but they *MUST NOT* advertise those owned bytes as loan-backed typed stable-container borrows.
--

[.specitem,oft-sid="dsn~utransport-loan-provenance-diagnostic~1",oft-needs="impl,utest",oft-tags="TransportLayerImpl"]
--
If a language binding exposes payload loan provenance, that provenance *MUST* be treated as diagnostic information and *MUST NOT* be the safety gate for typed payload borrowing. Public provenance values *SHOULD* remain semantic, for example opaque transport loan or shared-memory loan, and *SHOULD NOT* become a backend-specific or test-stub-specific taxonomy.
--
...
[.specitem,oft-sid="dsn~utransport-streamer-copy-minimized-scope~1",oft-needs="impl,utest",oft-tags="TransportLayerImpl"]
--
...
[.specitem,oft-sid="dsn~utransport-owned-protocol-zero-copy-claims~1",oft-needs="impl,utest",oft-tags="TransportLayerImpl"]
--
MQTT 5 and SOME/IP/vSomeIP native-frame transports are owned native-frame transports in this revision. They *MUST* preserve required uProtocol metadata and payload bytes according to their protocol binding, but they *MUST NOT* advertise direct true-zero-copy transmit, direct true-zero-copy receive or loan-backed stable-container typed borrowing.
--
