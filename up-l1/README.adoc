...
The UML below defines the abstract transport contract in terms of `UMessage`. Language bindings that expose native frame APIs MAY split this contract into owned-buffer and zero-copy capability interfaces, provided they preserve the same uProtocol metadata, payload bytes, filtering, and delivery semantics.
