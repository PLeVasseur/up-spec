...
[#cloudevents-transport-profile]
== Transport Profile Role

CloudEvents is a whole-frame envelope profile for carrying uProtocol message metadata and payload through CloudEvents formats and bindings. It is not itself a transport runtime, but a uP-L1 representation profile that transport libraries can use when binding uProtocol messages to CloudEvents-compatible transports.

The CloudEvents profile:

* *MUST* preserve uProtocol metadata using the attribute mapping in <<uAttributes Mapping>>.
* *MUST* preserve payload bytes and payload encoding information using <<uPayload Mapping>>.
* *MUST* treat the CloudEvents event as a whole-frame envelope that carries uProtocol metadata and payload together.
* *MUST* document which CloudEvents binding supplies the actual transport send, receive, listener, filtering, and delivery behavior.
* *MUST NOT* claim native-frame or loan-backed zero-copy capability unless a concrete transport binding documents a separate capability surface and its copy boundaries.
