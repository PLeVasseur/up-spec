...
[.specitem,oft-sid="dsn~cloudevents-pformat-without-datacontenttype~1",oft-needs="impl,utest",oft-tags="TransportLayerImpl"]
--
When a UMessage payload format is mapped through the custom `pformat` property, the CloudEvent *MUST NOT* also set `datacontenttype` solely from that UPayloadFormat value. A transport binding *MAY* set `datacontenttype` only when it carries an independent CloudEvents media type outside the UPayloadFormat-to-`pformat` mapping.
--
