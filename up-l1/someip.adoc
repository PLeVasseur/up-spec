...
This prefix is a transport binding artifact. It *MUST NOT* be modeled as a generated Protocol Buffers transport envelope and *MUST NOT* constrain the application payload `WireFormat`. The application payload begins after the native frame prefix and remains exactly the bytes produced by the selected serializer.

For Rust `vsomeip` interoperability, the native frame prefix is:
...
someip_native_frame =
  ...
  encoding_format_id: string
  encoding_content_type: string
  encoding_schema_ref: string   // empty string means absent; present schema refs are non-empty
...
Implementations *MUST* reject unknown magic values, unsupported versions, malformed UTF-8, invalid optional markers, invalid UUIDs, and oversized metadata fields. Implementations *MUST* reconstruct the original native frame metadata before delivering the frame to uProtocol application code.
