...
- `[PRS_SOMEIP_XXXXX]` refer to https://some-ip.com/standards.shtml[AUTOSAR SOME/IP specification].
- `[PRS_SOMEIP_SD_XXXXX]` refer to https://some-ip.com/standards.shtml[SOME/IP Service Discovery Protocol Specification].
...
==== Native Frame Payload Prefix for vSOMEIP Bindings

Some SOME/IP bindings, including `vsomeip`, expose only the SOME/IP payload bytes to language-level transport implementations and do not expose a protocol-native metadata channel large enough to preserve all uProtocol frame metadata. Such bindings *MAY* encode a native frame prefix inside the SOME/IP payload.

This prefix is a transport binding artifact. It *MUST NOT* be modeled as a generated Protocol Buffers transport envelope and *MUST NOT* constrain the application payload format. The application payload begins after the native frame prefix and remains exactly the bytes produced by the selected serializer.

For `vsomeip` native-frame interoperability, the binding prefix is:
...
someip_native_frame =
  ...
  application_payload: bytes    // remaining bytes after the prefix
...
If `encoding` is absent, `application_payload` *MUST* be empty and the delivered frame has no payload. If `encoding` is present, `application_payload` is present and may be empty. Implementations *MUST* reject unknown magic values, unsupported versions, malformed UTF-8, invalid optional markers, invalid UUIDs, oversized metadata fields, and non-empty application payload bytes without encoding metadata. Implementations *MUST* reconstruct the original native frame metadata before delivering the frame to uProtocol application code.

SOME/IP/vSomeIP is an owned native-frame transport in this revision. SOME/IP bindings that carry stable-container payloads through this native-frame prefix *MUST* preserve the `up.stable-container` custom payload encoding metadata and application payload bytes exactly. A binding that copies payload bytes through SOME/IP or C++ library storage *MUST NOT* advertise direct true-zero-copy transmit, direct true-zero-copy receive, or loan-backed stable-container typed borrowing.

==== Native-Frame Conformance Expectations

SOME/IP native-frame conformance tests *MUST* cover prefix version and magic rejection, invalid native attributes, malformed UTF-8 rejection, optional-marker rejection, oversized metadata rejection, custom payload encoding preservation, standard payload encoding preservation, stable-container metadata preservation without typed borrowing, payload/encoding mismatch rejection, and exact exclusion of the native-frame prefix from the delivered application payload. Native vSomeIP runtime smoke tests *SHOULD* run where the middleware installation and network fixture are available.

[.specitem,oft-sid="dsn~up-transport-someip-native-frame-conformance~1",oft-needs="impl,utest",oft-tags="TransportLayerImpl"]
--
SOME/IP native-frame bindings that use an in-payload metadata prefix *MUST* prove that the prefix is validated as transport metadata, hidden from application payload bytes, and compatible only with owned/copying transport semantics unless a separate proven loaned path exists.
--
...
^.^h| ServiceID xref:#note-1[^(1)^] ^.^h| MethodID ^.^h| ClientID xref:#note-1[^(1)^],xref:#note-4[^(4)^] ^.^h| SessionID

| NOTIFICATION xref:#note-5[^(5)^] | REQUEST_NO_RETURN (0x01) | sink.ue_id   | sink.resource_id | source.ue_id | xref:#note-2[^(2)^],xref:#note-3[^(3)^] | sink.ue_major_version   |
| PUBLISH       | NOTIFICATION (0x02)      | source.ue_id  | source.resource_id | sink.ue_id=0  | xref:#note-2[^(2)^]       | source.ue_major_version |
| REQUEST       | REQUEST (0x00)           | sink.ue_id    | sink.resource_id   | source.ue_id  | xref:#note-2[^(2)^],xref:#note-3[^(3)^] | sink.ue_major_version   | OK

.2+.^| RESPONSE .^| RESPONSE (0x80) | sink.ue_id  | sink.resource_id | source.ue_id | xref:#note-2[^(2)^],xref:#note-3[^(3)^] | sink.ue_major_version | OK
                .^| ERROR (0x81)    | sink.ue_id  | sink.resource_id | source.ue_id | xref:#note-2[^(2)^],xref:#note-3[^(3)^] | sink.ue_major_version |  <<commstatus-error-mapping,commstatus>>
|===
