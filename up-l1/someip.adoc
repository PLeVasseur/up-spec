...
* SOME/IP `NOTIFICATION` *SHOULD* use the xref:../basics/upriority.adoc[UPriority.CS1] priority.
* SOME/IP `REQUEST` *SHOULD* use xref:../basics/upriority.adoc[UPriority.CS4] or higher priority.
...
| payload_encoding_id | SOME/IP does not provide a field for the xref:../basics/payload_encoding_registry.adoc[payload-encoding identifier].
...
| Publish | NOTIFICATION | Publish SOME/IP Events.
| Notification | n/a | Same as SOME/IP `NOTIFICATION`.
| RpcRequest | REQUEST | RPC Request
| RpcResponse | RESPONSE or ERROR | RPC Response or Error has occurred while attempting to deliver the message.
|===
...
** *MUST* clean up the cache after the `ttl` has expired and send an Error `UMessageType.RpcResponse`
...
| RpcRequest | REQUEST (0x00)           | sink.ue_id    | sink.resource_id   | source.ue_id  | xref:#note-2[^(2)^],xref:#note-3[^(3)^] | sink.ue_major_version   | OK

.2+.^| RpcResponse .^| RESPONSE (0x80) | sink.ue_id  | sink.resource_id | source.ue_id | xref:#note-2[^(2)^],xref:#note-3[^(3)^] | sink.ue_major_version | OK
...
| Cancelled | E_NOT_OK
| Unknown | E_NOT_OK
| InvalidArgument | E_WRONG_MESSAGE_TYPE / E_UNKNOWN_METHOD
| DeadlineExceeded | E_TIMEOUT
| NotFound | E_UNKNOWN_SERVICE
| AlreadyExists | E_NOT_OK
| PermissionDenied | E_NOT_OK
| Unauthenticated | E_NOT_OK
| ResourceExhausted | E_NOT_OK
| FailedPrecondition | E_WRONG_PROTOCOL_VERSION / E_WRONG_INTERFACE_VERSION
| Aborted | E_NOT_OK
| OutOfRange | E_NOT_OK
| Unimplemented | E_NOT_OK
| Internal | E_NOT_REACHABLE
| Unavailable | E_NOT_READY
| DataLoss | E_MALFORMED_MESSAGE
|===
