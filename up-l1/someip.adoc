...
SOME/IP native-frame conformance tests SHOULD cover prefix version and magic rejection, malformed UTF-8 rejection, optional-marker rejection, oversized metadata rejection, custom payload encoding preservation, standard payload encoding preservation, stable-container metadata preservation without typed borrowing, payload/encoding mismatch rejection, and exact exclusion of the native-frame prefix from the delivered application payload.
