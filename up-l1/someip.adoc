...
xref:./README.adoc#utransport[Transport layer (uP-L1)] specifies how xref:../basics/umessage.adoc[uMessages] are sent and received over a _wire_ between *uEs* or between devices. In this section we will cover the SOME/IP communication protocol binding and event mapping.
...
SOME/IP/vSomeIP is an owned native-frame transport in this revision. Preserving uProtocol metadata and payload bytes through a SOME/IP binding does not provide a transport loan or receive lease. SOME/IP/vSomeIP transport implementations *MUST NOT* advertise direct true-zero-copy transmit, direct true-zero-copy receive, or loan-backed stable-container typed borrowing.
