...
SOME/IP binding supports the *classic* transport family only. Frame-oriented
family operations *MUST* fail with an explicit, typed unsupported-operation
status. Metadata carriage pattern — Header mapping: attributes derive from
and reconstruct out of SOME/IP header fields, with the reconstruction
conventions documented in the mapping notes of this document; deployments
without metadata carriage use the assumed-encoding rules of
...
SOME/IP *MUST* define a binding profile. The binding *MUST* declare whether
UFrame metadata is carried in a native SOME/IP placement, inside a
`UPFE` whole-frame envelope, or by another explicitly named transport-specific
placement.

The binding *MUST* declare the accepted metadata profile or profiles. New native
selected-wire use *SHOULD* accept the canonical metadata field block v1. The
...
and routing mirrors that do not match the SOME/IP service, instance, method, and
event routing fields selected by the binding. These failures *MUST NOT* be
interpreted as ordinary payload decode failures.
