...
[#someip-transport-capability-profile]
== Transport Capability Profile

The SOME/IP binding is a uP-L1 transport translation profile. It maps uProtocol addresses, message metadata, payload information, request/response correlation, and communication status to SOME/IP fields and service-discovery behavior.

The SOME/IP profile:

* *MUST* preserve the compatibility message transport semantics required by uP-L1 when translating between uProtocol messages and SOME/IP messages.
* *MUST* preserve source, sink, message type, request/response correlation, and communication status according to the mappings in this document.
* *MUST* treat SOME/IP service identifiers, instance identifiers, method identifiers, event identifiers, and session identifiers as transport-local representation, not as Communication Layer ownership of wire identity.
* *MUST* document which payload encoding information is preserved, inferred from service configuration, ignored, or unsupported.
