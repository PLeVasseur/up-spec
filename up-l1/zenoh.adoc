...
Ordinary Zenoh payload transport is not by itself a loan-backed receive contract. A Zenoh shared-memory transport implementation *MAY* advertise direct true-zero-copy transmit or receive only for APIs that write directly into Zenoh shared-memory transmit storage or borrow directly from a Zenoh shared-memory receive lease without allocation, payload coalescing or copied fallback storage.
