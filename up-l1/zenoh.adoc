...
A uProtocol message consists of _UAttributes_ and optional payload. Native frame language bindings also carry explicit _PayloadEncoding_ metadata for the payload wire representation. The following sections define how these are mapped to/from Zenoh Resources.
...
Native-frame language bindings *MAY* use a binding-defined Zenoh attachment representation instead of a serialized generated `UAttributes` blob, provided the representation preserves the same abstract uProtocol message semantics. Such a representation *MUST* preserve all native `UAttributes` fields required by xref:wire-format-and-frame-api.adoc[Wire Format and Frame API] and *MUST* preserve optional payload encoding metadata across the Zenoh boundary.

Rust binding note: for native-frame interoperability, the attachment representation is:

[source]
----
zenoh_native_frame_attachment =
  uprotocol_major_version: uint8 = 1
  magic[4] = "UFRM"
  id_msb: uint64 little-endian
  id_lsb: uint64 little-endian
  message_type: uint8
  priority: uint8
  ttl: optional<uint32 little-endian>
  source_uri: string
  sink_uri: string              // empty string means absent
  encoding: optional<encoding>
  request_id: optional<uuid>
  traceparent: optional<string>
  token: optional<string>
  permission_level: optional<uint32 little-endian>
  commstatus: optional<uint8>
...
Implementations *MUST* reject unknown magic values, malformed UTF-8, invalid optional markers, invalid UUIDs, trailing attachment bytes, and payload bytes without encoding metadata. Implementations *MUST* reconstruct the same native frame metadata before delivering the frame to uProtocol application code. If encoding metadata is absent, the Zenoh payload *MUST* be empty and the delivered frame has no payload.
...
For native-frame language bindings, the Zenoh payload *MUST* contain exactly the application payload bytes produced by the selected serializer. Binding-specific attachment metadata *MUST NOT* be exposed as part of the frame payload.
--

=== Zero-Copy Payload Access

Zenoh represents payload bytes as `ZBytes`. `ZBytes` can be backed by one or more byte slices and can also represent shared-memory-backed buffers when Zenoh's shared-memory support is enabled.

[.specitem,oft-sid="dsn~up-transport-zenoh-zero-copy-zbytes-slices~1",oft-needs="impl,utest",oft-tags="TransportLayerImpl"]
--
A Zenoh zero-copy receive implementation *MUST* expose payload bytes from `ZBytes` using the ordered slice view. It *MUST NOT* call a `ZBytes` API that allocates and coalesces segmented payload storage into a contiguous buffer except when explicitly adapting the message to an owned-frame API.
--

Ordinary Zenoh payload transport is not by itself a loan-backed receive contract. Zenoh transports that receive stable-container payloads *MUST* preserve the `up.stable-container` custom payload encoding metadata. They *MUST NOT* expose typed stable-container borrowing unless the binding can prove the relevant `ZBytes` view is contiguous, aligned to the stable-container `align` parameter, loan-backed for the receive lease lifetime, and not produced by a hidden coalescing copy. Rust binding note: stable-container typed receive uses a loan-backed stable receive method rather than an owned-frame helper.

[.specitem,oft-sid="dsn~up-transport-zenoh-shm-feature-gate~1",oft-needs="impl,utest",oft-tags="TransportLayerImpl"]
--
Zenoh shared-memory transmit support *MUST* be feature-gated or otherwise explicitly opted into, because the underlying Zenoh shared-memory APIs require Zenoh's shared-memory support and may depend on unstable Zenoh APIs.
--

=== Native-Frame Conformance Expectations

Zenoh native-frame conformance tests *MUST* cover attachment round trips for required and optional native attributes, invalid native attributes, standard payload encoding preservation, custom payload encoding preservation, malformed attachment rejection, payload bytes without encoding rejection, and hidden attachment exclusion from the delivered payload.

Zero-copy receive conformance tests *MUST* additionally cover segmented `ZBytes` delivery without hidden coalescing, rejection or non-advertisement of typed stable-container borrowing for non-SHM or non-contiguous payloads, and successful typed stable-container borrowing only when shared-memory provenance, contiguity, alignment, and receive-lease lifetime are proven. Native shared-memory smoke tests *SHOULD* run where the Zenoh SHM environment is available.

[.specitem,oft-sid="dsn~up-transport-zenoh-native-frame-conformance~1",oft-needs="impl,utest",oft-tags="TransportLayerImpl"]
--
Zenoh native-frame bindings *MUST* prove that attachment metadata is preserved separately from application payload bytes and that strict zero-copy receive paths do not turn segmented or copied `ZBytes` storage into advertised loan-backed typed borrows.
--
