...
Zenoh native-frame conformance tests SHOULD cover attachment round trips for required and optional `UAttributes`, standard payload encoding preservation, custom payload encoding preservation, malformed attachment rejection, payload bytes without encoding rejection, and hidden attachment exclusion from the delivered payload.

Zero-copy receive conformance tests SHOULD additionally cover segmented `ZBytes` delivery without hidden coalescing, rejection or non-advertisement of typed stable-container borrowing for non-SHM or non-contiguous payloads, and successful typed stable-container borrowing only when shared-memory provenance, contiguity, alignment, and receive-lease lifetime are proven.
