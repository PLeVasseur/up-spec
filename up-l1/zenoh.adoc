...
Native-frame language bindings *MAY* use a binding-defined Zenoh attachment representation instead of a serialized generated `UAttributes` blob, provided the representation preserves the same abstract uProtocol message semantics. Such a representation *MUST* preserve all native `UAttributes` fields required by xref:wire-format-and-frame-api.adoc[Wire Format and Frame API] and *MUST* preserve `UEncoding.format_id`, `UEncoding.content_type`, and any non-empty `UEncoding.schema_ref` across the Zenoh boundary.

For Rust native-frame interoperability, the attachment representation is:
...
Implementations *MUST* reject unknown magic values, malformed UTF-8, invalid optional markers, invalid UUIDs, and trailing attachment bytes. Implementations *MUST* reconstruct the same native frame metadata before delivering the frame to uProtocol application code.
...
For native-frame language bindings, the Zenoh payload *MUST* contain exactly the application payload bytes produced by the selected `WireFormat` serializer. Binding-specific attachment metadata *MUST NOT* be exposed as part of the frame payload.
--
