...
[#zenoh-transport-capability-profile]
== Transport Capability Profile

The Zenoh binding is a uP-L1 transport profile that maps uProtocol metadata to Zenoh attachments, payload bytes to the Zenoh payload, and routing/filtering information to Zenoh key expressions.
...
* *MUST* preserve uProtocol metadata using the attachment mapping in <<UAttributes>> and payload bytes using <<Payload Mapping>>.
* *MUST* treat key expressions as transport-local routing and filtering representation, not as Communication Layer ownership of wire identity.
* *MAY* expose native-frame, shared-memory, or loan-backed capabilities when a concrete transport library can preserve the selected wire profile, filtering behavior, payload-family checks, and compatibility adaptation rules defined by uP-L1.
* *MUST* document any copy boundary between Zenoh attachments, Zenoh payload storage, native frames, loan-backed storage, and complete `UMessage` values.
