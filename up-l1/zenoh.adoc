...
Ordinary Zenoh payload transport is not by itself a loan-backed receive contract. Zenoh transports that receive stable-container payloads *MUST* preserve the `up.stable-container` custom payload encoding metadata. They *MUST NOT* expose typed stable-container borrowing unless the binding can prove the relevant `ZBytes` view is contiguous, aligned to the stable-container `align` parameter, loan-backed for the receive lease lifetime, and not produced by a hidden coalescing copy. Rust stable-container typed receive MUST use a loan-backed stable receive method such as `borrow_stable_payload<T>()`.
