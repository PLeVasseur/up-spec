...
A uProtocol message consists of _UAttributes_ and optional payload. Native frame language bindings also carry explicit _UEncoding_ metadata for the payload wire representation. The following sections define how these are mapped to/from Zenoh Resources.
...
encoding =
  format_id: string
  content_type: string
  schema_ref: string  // empty string means absent; present schema refs are non-empty
----
