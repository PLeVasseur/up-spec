...
A native uProtocol frame consists of _UAttributes_, wire-format metadata, and optional payload bytes. The following sections define how these are mapped to/from iceoryx2 Services without requiring a generated Protocol Buffers envelope.
...
a| *MUST* describe the payload loan size and requested serializer alignment.
...
* `UEncoding.format_id`;
* `UEncoding.content_type`;
* non-empty `UEncoding.schema_ref`, when present;
...
optional<string> =
  ...
  string              // present only when present == 1
----

Implementations *MUST* reject metadata prefixes with an unknown magic value, malformed UTF-8, invalid optional markers, trailing bytes, or lengths that exceed the sample payload. Regardless of representation, the transport API *MUST* reconstruct the same native frame metadata on receive.
...
--
An iceoryx2 `Sample` that is used to convey a uProtocol frame *MUST* expose through the uProtocol transport API the frame payload bytes exactly as produced by the selected `WireFormat` serializer.

An implementation *MAY* reserve metadata prefix bytes before the frame payload bytes in the underlying iceoryx2 sample payload. If it does so, those bytes *MUST* be hidden from `UTxBuffer.payload_mut()` and `UZeroCopyRxFrame.payload()`.
--

[.specitem,oft-sid="dsn~up-transport-iceoryx2-payload-zero-copy~1",oft-needs="impl,utest",oft-tags="TransportLayerImpl"]
--
An iceoryx2 frame that is used to convey a uProtocol frame *MUST* be zero-copy and shared-memory compatible:
