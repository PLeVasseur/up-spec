...
Variable metadata that is not naturally fixed-size for iceoryx2 user headers, such as `UEncoding`, `traceparent`, and `token`, SHOULD be encoded in an implementation metadata sidecar rather than forcing arbitrary fixed user-header capacities. The sidecar length is carried by `metadata_len`. The sidecar is transport metadata and *MUST NOT* be exposed as part of the frame payload by the language transport API.

==== Implementation Metadata Sidecar

The implementation metadata sidecar preserves variable frame metadata while keeping the iceoryx2 user header small and cheap to inspect.
...
The exact binary representation is language-binding specific unless and until this document standardizes cross-language iceoryx2 sidecar interoperability. Regardless of representation, the transport API *MUST* reconstruct the same native frame metadata on receive.
...
An implementation *MAY* reserve metadata sidecar bytes before the frame payload bytes in the underlying iceoryx2 sample payload. If it does so, those bytes *MUST* be hidden from `UTxBuffer.payload_mut()` and `UZeroCopyRxFrame.payload()`.
--
