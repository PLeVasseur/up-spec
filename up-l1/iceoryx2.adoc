...
[source,rust]
----
const MAX_AUTHORITY_NAME_LENGTH: usize = 128;
#[repr(C)]
#[derive(ZeroCopySend)]
struct UProtocolHeader {
    ...
    commstatus: u8,
    source_authority: FixedSizeVec<u8, MAX_AUTHORITY_NAME_LENGTH>,
    source_ue_id: u32,
    source_ue_version_major: u32,
    source_resource_id: u32,
    sink_present: u8,
    sink_authority: FixedSizeVec<u8, MAX_AUTHORITY_NAME_LENGTH>,
    sink_ue_id: u32,
...
| `source_*`
a| *MUST* be set from the native source `UUri`. Numeric URI components SHOULD be stored as numeric header fields instead of serializing the URI string.

| `sink_present`, `sink_*`
a| *MUST* indicate whether a sink exists. When present, the sink fields *MUST* be set from the native sink `UUri`. When absent, `sink_present` *MUST* be zero and sink fields *MUST* be ignored.
...
The user header *MUST NOT* contain a serialized generated `UAttributes` blob. Transports SHOULD project native attributes into fields that are cheap for the transport to inspect. URI entity/version/resource components are numeric, and authority names are bounded by the uProtocol authority-name constraint.

Variable metadata that is not naturally fixed-size for iceoryx2 user headers, such as `UEncoding`, `traceparent`, and `token`, SHOULD be encoded in an implementation metadata prefix rather than forcing arbitrary fixed user-header capacities. The prefix length is carried by `metadata_len`. The prefix is transport metadata and *MUST NOT* be exposed as part of the frame payload by the language transport API.
