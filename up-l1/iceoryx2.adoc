...
[#iceoryx2-transport-capability-profile]
== Transport Capability Profile

The iceoryx2 binding is a uP-L1 transport profile for shared-memory service communication. It can support compatibility message transport and loan-backed native-frame capability when the binding preserves the metadata, payload, filtering, and delivery semantics defined by uP-L1.

The iceoryx2 profile:

* *MUST* preserve uProtocol metadata using the service user-header mapping in <<UAttributes>>.
* *MUST* preserve payload bytes using <<Payload Mapping>> and reject payload representations that cannot satisfy the declared shared-memory requirements.
* *MUST* state which bytes are placed in the service user header, which bytes are in the sample payload, and which adapter boundaries copy metadata or payload data.
* *MUST* treat service names as transport-local routing and filtering representation, not as Communication Layer ownership of wire identity.
* *MAY* expose loan-backed transmit or receive capability only for directions and storage that are actually loaned by the transport runtime.
* *MUST* provide or document a compatibility adapter when complete `UMessage` values are required.
...
A language binding *MAY* express these shared-memory requirements with marker mechanisms, annotations, generated-code contracts, or schema restrictions. Such binding-specific mechanisms are non-normative unless this document names them as part of the interoperable iceoryx2 mapping.
