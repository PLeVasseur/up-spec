...
Language libraries that expose native frame or zero-copy APIs instead of a generated message envelope at their public transport boundary

[.specitem,oft-sid="dsn~up-language-native-frame-api~1",oft-needs="impl,uman",oft-title="Native frame APIs preserve abstract message semantics",oft-tags="LanguageLibrary"]
--
* *MUST* document how the binding-level frame representation maps to the abstract uProtocol message metadata, payload bytes, filtering rules, and delivery semantics defined by the Transport Layer API.
* *MUST NOT* require transport implementations to wrap application payload bytes in a format-specific generated envelope when the same metadata can be represented by the binding-level frame type.
* *MUST* document whether payload bytes and payload encoding metadata are optional and whether the binding defines an explicit raw-bytes default when payload bytes are present without encoding metadata.
* *MUST* document any adapter that copies between owned-buffer and zero-copy transport capabilities, including the copy boundary and whether the adapter is intended for routing rather than end-to-end zero-copy delivery.
--

Language libraries that expose wire formats or wire-bound native-frame transports

[.specitem,oft-sid="dsn~up-language-wire-format-typed-support~1",oft-needs="impl,uman",oft-title="Wire-format typed support is binding-neutral",oft-tags="LanguageLibrary"]
--
* *MUST* document how a selected wire format maps native frame metadata and payload encoding metadata at the transport boundary.
* *MUST* document how typed payload support is checked for each claimed operation, such as static type checking, compile-time validation, code-generation validation, or equivalent binding-time validation.
* *MUST* document dynamically configured wire-format paths as runtime-checked paths, not as static selected-wire guarantees.
--

Language libraries that expose multiple public API planes from one package or namespace

[.specitem,oft-sid="dsn~up-language-public-surface-tiers~1",oft-needs="impl,uman",oft-title="Public surfaces are tiered by audience and layer",oft-tags="LanguageLibrary"]
--
* *MUST* document which entry points are the ordinary application/service Communication Layer APIs, compatibility message transport APIs, native frame, wire-format, or zero-copy transport capability APIs, expert caller-obligation APIs, and test/proof/fixture support APIs.
* *MUST* keep compatibility message transport APIs documented as valid APIs unless the language library explicitly removes or replaces them through a versioned migration.
* *MUST* keep documented direct access paths for exposed Transport Layer capability APIs, including compatibility message transport, native frame, wire-format, and zero-copy capability surfaces. Presenting an ordinary Communication Layer entry point *MUST NOT* make those direct Transport Layer APIs hard to find or imply that direct Transport Layer usage is unsupported.
* *MUST NOT* describe wire-format identity, selected-wire profile identity, metadata encoding, or payload-family checks as owned by Communication Layer roles. Communication Layer APIs consume configured transport endpoints through messaging roles.
* *SHOULD* present expert caller-obligation APIs and test/proof/fixture support separately from ordinary application/service examples.
--
